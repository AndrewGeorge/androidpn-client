...
public final class ServiceManager {
	...
	public void setTags(final List<String> tagslist) {
		final String username = sharedPrefs.getString(Constants.XMPP_USERNAME,
				"");
		if (TextUtils.isEmpty(username) || tagslist.isEmpty()
				|| tagslist == null) {
			return;
		}
				...
				if (xmppManager != null) {
					if (!xmppManager.isAuthenticated()) {
						...
						synchronized (xmppManager) {
							try {
								Log.d(LOGTAG, "wait for isAuthenticated.. ");
								xmppManager.wait();
					...
					Log.d(LOGTAG, "Authenticated allady send SetTagsIq now..");
					SetTagsIQ settagiq = new SetTagsIQ();
					settagiq.setType(IQ.Type.SET);
					settagiq.setTaglist(tagslist);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (xmppManager != null) {
					...
					Log.d(LOGTAG, "Authenticated allady send SetAliasIq now..");
					SetAliasIQ setiq = new SetAliasIQ();
					setiq.setType(IQ.Type.SET);
					setiq.setAlias(alias);
					setiq.setUsername(username);
					xmppManager.getConnection().sendPacket(setiq);
				}
